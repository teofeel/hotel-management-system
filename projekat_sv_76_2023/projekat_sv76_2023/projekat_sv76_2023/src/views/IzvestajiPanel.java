...
public class IzvestajiPanel extends JPanel{
	...
	private void setSobeTable(String pocetak, String kraj) {
		...
		String[] columnNamesSobe = {"Sifra Sobe","Tip Sobe", "Nocenja", "Prihodi"};
		Object[][] data = new Object[sobeNocenja.size()][4];
		...
		for(Map.Entry<Integer, Float> entry:sobePrihodi.entrySet()) {
            ...
            data[index][1] = SobaManager.sobe.get(sifraSobe).getNazivSobe();
            data[index][2] = nocenja != null ? nocenja : 0;
            data[index][3] = prihodi != null ? prihodi : 0.0f;
            index++;
		}
