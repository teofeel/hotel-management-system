...
public class SobaManager {
	...
	public ArrayList<Soba> slobodneSobe(Rezervacija rez) {
		...
		for (Rezervacija rezervacija : RezervacijaManager.rezervacije) {
			if(rezervacija.getStatus().equals(StatusRezervacije.POTVRDJENA) && rez.getTipSobe().getNaziv().equals(rezervacija.getTipSobe().getNaziv()) 
					&& rezervacija.getDatumDolaska().equals(rez.getDatumDolaska()) && rezervacija.getDatumOdlaska().equals(rez.getDatumOdlaska())) {
				sobePoTipu.remove(0);
			}
	...
	public static boolean pregledSlobodnihSoba(LocalDate pocetak, LocalDate kraj, String tipSobe) {
		...
		for (Soba soba : SobaManager.sobe.values()) {
			if(soba.getNazivSobe().equals(tipSobe) && (soba.getStatus().equals(StatusSobe.SLOBODNA) || soba.getStatus().equals(StatusSobe.SPREMANJE))) {
				sobePoTipu.add(soba);
			}
		}
		if (sobePoTipu.isEmpty()) {
