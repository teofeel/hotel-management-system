	...
	public HashMap<Integer, Integer> sobaNocenja(String p, String k) {
		try {
			...
			for(Rezervacija rez:RezervacijaManager.rezervacije) {
				...
				if (rez.getDatumOdlaska().isBefore(pocetak) || rez.getDatumDolaska().isAfter(kraj)) continue;
				
				int broj = sobeNocenja.containsKey(rez.getSoba().getSifra()) ? sobeNocenja.get(rez.getSoba().getSifra()):0;
				
				if (rez.getDatumDolaska().isBefore(pocetak) && rez.getDatumOdlaska().isAfter(kraj)) {
					int i = Period.between(pocetak, kraj).getDays();
					broj+=i;
				}
				else if(rez.getDatumDolaska().isBefore(pocetak) && rez.getDatumOdlaska().isBefore(kraj)) {
					int i =Period.between(pocetak, rez.getDatumOdlaska()).getDays();
					broj+=i;
				}
				else if(rez.getDatumDolaska().isAfter(pocetak) && rez.getDatumOdlaska().isBefore(kraj)) {
					int i = Period.between(rez.getDatumDolaska(), rez.getDatumOdlaska()).getDays();
					broj+=i;
				}
				else if(rez.getDatumDolaska().isBefore(kraj) && rez.getDatumOdlaska().isAfter(kraj)) {
					int i= Period.between(rez.getDatumDolaska(), kraj).getDays();
					broj+=i;
				}
				else if(rez.getDatumDolaska().equals(pocetak) && rez.getDatumOdlaska().isBefore(kraj.plusDays(1))) {
					int i= Period.between(pocetak, rez.getDatumOdlaska()).getDays();
					broj+=i;
				}else if(rez.getDatumDolaska().isAfter(pocetak.minusDays(1)) && rez.getDatumOdlaska().equals(kraj)) {
					int i= Period.between(rez.getDatumDolaska(), kraj).getDays();
					broj+=i;
				}
	...
	public HashMap<Integer, Float> sobaPrihodi(String p, String k) {
		try {
			...
			for(Rezervacija rez:RezervacijaManager.rezervacije) {
				...
				int rezDani = Period.between(rez.getDatumDolaska(), rez.getDatumOdlaska()).getDays();
				
				if (rez.getDatumDolaska().isBefore(pocetak) && rez.getDatumOdlaska().isAfter(kraj)) {
					int dani = Period.between(pocetak, kraj).getDays();
					float procenat = (float) dani / (float) rezDani;
					prihod+=rez.getCena()  * procenat;
				}
				else if(rez.getDatumDolaska().isBefore(pocetak) && rez.getDatumOdlaska().isBefore(kraj)) {
					int dani =Period.between(pocetak, rez.getDatumOdlaska()).getDays();
					float procenat = (float) dani / (float) rezDani;
					prihod+=rez.getCena()  * procenat;
				}
				else if(rez.getDatumDolaska().isAfter(pocetak) && rez.getDatumOdlaska().isBefore(kraj)) {
					int dani = Period.between(rez.getDatumDolaska(), rez.getDatumOdlaska()).getDays();
					float procenat = (float) dani / (float) rezDani;
					prihod+=rez.getCena()  * procenat;
				}
				else if(rez.getDatumDolaska().isBefore(kraj) && rez.getDatumOdlaska().isAfter(kraj)) {
					int dani = Period.between(rez.getDatumDolaska(), kraj).getDays();
					float procenat = (float) dani / (float) rezDani;
					prihod+=rez.getCena()  * procenat;
				}
				else if(rez.getDatumDolaska().equals(pocetak) && rez.getDatumOdlaska().isBefore(kraj.plusDays(1))) {
