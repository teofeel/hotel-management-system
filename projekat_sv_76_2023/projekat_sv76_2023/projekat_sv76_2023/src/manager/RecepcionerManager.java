...
public class RecepcionerManager {
		...
		try {
			for(int i=0;i<RezervacijaManager.rezervacije.size();i++) {
				if(RezervacijaManager.rezervacije.get(i).getGost().equals(gost) && RezervacijaManager.rezervacije.get(i).getDatumDolaska().equals(datumDolaska) 
						&& RezervacijaManager.rezervacije.get(i).getDatumOdlaska().equals(datumOdlaska) && RezervacijaManager.rezervacije.get(i).getTipSobe().getNaziv().equals(soba.getNazivSobe())) {
	...
	public String izmenaStatusaRezrvacije(String gost, String datumDolaska, String datumOdlaska, String tipSobe, StatusRezervacije status) {
		try {
			...
			for(int i=0;i<RezervacijaManager.rezervacije.size();i++) {
				if(RezervacijaManager.rezervacije.get(i).getGost().equals(gost) && RezervacijaManager.rezervacije.get(i).getDatumDolaska().toString().equals(datumDolaska) 
						&& RezervacijaManager.rezervacije.get(i).getDatumOdlaska().toString().equals(datumOdlaska) && RezervacijaManager.rezervacije.get(i).getTipSobe().getNaziv().equals(tipSobe) 
						&& RezervacijaManager.rezervacije.get(i).getStatus().equals(StatusRezervacije.NA_CEKANJU)) {
					
					
					if(LocalDate.parse(datumOdlaska).equals(LocalDate.parse(datumDolaska))) {
						RezervacijaManager.rezervacije.get(i).setStatus(StatusRezervacije.ODBIJENA);
						throw new Exception("Nije moguce prihvatiti rezervaciju");
					}
